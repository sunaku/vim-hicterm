function! HiCterm(...) abort
  for color in range(0, (has('gui_running') ? 256 : &t_Co) - 1)
    ...
    if a:0 > 0
      ...
      execute 'highlight' group join(map(copy(a:000), '"cterm" . v:val . "=' . color . '"'), ' ')
          \ join(map(copy(a:000), '"gui" . v:val . "=' . s:xterm_colors[color] . '"'), ' ')
    endif
  endfor
endfunction
...
" From <http://github.com/guns/xterm-color-table.vim> (MIT license)
let s:xterm_colors = {
